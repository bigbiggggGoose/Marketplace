...
public class TradeRequestController {
    ...
    public ResponseEntity<?> getMyRequests(HttpServletRequest httpRequest) {
        ...
        return tradeRequestService.getMyRequests(userId);
    }
    ...
    public ResponseEntity<?> getReceivedRequests(HttpServletRequest httpRequest) {
        ...
        return tradeRequestService.getReceivedRequests(userId);
    }
    ...
    public ResponseEntity<?> sendTradeMessage(
            @PathVariable Long requestId,
            @RequestBody com.example.marketplace.dto.TradeMessageRequest request,
            HttpServletRequest httpRequest) {
        Long userId = getUserIdFromRequest(httpRequest);
        if (userId == null) {
            return ResponseEntity.status(401).body(java.util.Map.of("error", "未授权"));
        }
        return tradeRequestService.sendTradeMessage(requestId, userId, request);
    }

    @GetMapping("/requests/{requestId}/messages")
    public ResponseEntity<?> getTradeMessages(
            @PathVariable Long requestId,
            HttpServletRequest httpRequest) {
        Long userId = getUserIdFromRequest(httpRequest);
        if (userId == null) {
        ...
        return tradeRequestService.getTradeMessages(requestId, userId);
    }
